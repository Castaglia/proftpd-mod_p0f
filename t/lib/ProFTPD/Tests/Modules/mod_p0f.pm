package ProFTPD::Tests::Modules::mod_p0f;
...
my $TESTS = {
  ...
  p0f_retr_allowed_chrooted => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },

  p0f_retr_denied_chrooted => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },
  ...
  p0f_list_no_arg_denied => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },

  p0f_list_with_path_denied => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },

  p0f_list_opts_only_denied => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },
  ...
  p0f_mlsd_no_arg_denied => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },

  p0f_mlsd_with_path_denied => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },
  ...
  p0f_mlst_no_path_denied => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },

  p0f_mlst_with_path_denied => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },
  ...
  p0f_nlst_no_arg_denied => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },

  p0f_nlst_with_path_denied => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },

  p0f_nlst_opts_only_denied => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },
  ...
  p0f_stat_no_arg_denied => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },

  p0f_stat_with_path_denied => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },
...
sub p0f_retr_allowed {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};

  my $config_file = "$tmpdir/p0f.conf";
  my $pid_file = File::Spec->rel2abs("$tmpdir/p0f.pid");
  my $scoreboard_file = File::Spec->rel2abs("$tmpdir/p0f.scoreboard");

  my $log_file = File::Spec->rel2abs('tests.log');

  my $auth_user_file = File::Spec->rel2abs("$tmpdir/p0f.passwd");
  my $auth_group_file = File::Spec->rel2abs("$tmpdir/p0f.group");
  ...
  my $sock_dir = File::Spec->rel2abs("$tmpdir/var/p0f");
  mkpath($sock_dir);

  # Make sure that, if we're running as root, that the home directory has
  # permissions/privs set for the account we create
  if ($< == 0) {
    unless (chmod(0755, $home_dir, $sock_dir)) {
      die("Can't set perms on $home_dir to 0755: $!");
    }

    unless (chown($uid, $gid, $home_dir, $sock_dir)) {
...
#  my $p0f_socket = "$sock_dir/p0f.sock";
  my $p0f_socket = "/tmp/p0f.sock";

  my $p0f_sigs = File::Spec->rel2abs("t/etc/modules/mod_p0f/p0f.fp");
...
sub p0f_retr_denied {
  ...
  # Make sure that, if we're running as root, the directories have
...
sub p0f_config_schema_table_cols {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
